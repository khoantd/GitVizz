...
const GraphComponent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      graphData.edges.forEach((edge) => {
        ...
        if (graph.hasNode(source) && graph.hasNode(target) && source !== target && !graph.hasEdge(source, target)) {
          try {
      ...
      sigmaRef.current = new Sigma(graph, containerRef.current, {
        ...
        nodeReducer: (node, data) => {
          const type = ['circle', 'square', 'diamond', 'equilateral', 'star'].includes(data.type) ? data.type : 'circle';
          return { ...data, type };
        }
      });
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', height: '600px' }} className="bg-background border border-border rounded-md" />
  );
};
