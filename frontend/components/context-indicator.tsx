...
export function ContextIndicator({
  ...
  const { query_analysis, nodes_selected, total_nodes_available, context_completeness, token_usage_estimate, processing_time_ms, context_nodes } = contextMetadata;

  const getIntentIcon = (intent: string) => {
    switch (intent) {
      case 'debugging': return <Zap className="h-4 w-4 text-red-500" />;
      case 'explanation': return <Info className="h-4 w-4 text-blue-500" />;
      case 'modification': return <Code2 className="h-4 w-4 text-orange-500" />;
      case 'implementation': return <Layers className="h-4 w-4 text-green-500" />;
  ...
  const getScopeColor = (scope: string) => {
    switch (scope) {
      ...
      case 'comprehensive': return 'bg-red-100 text-red-800 border-red-200';
  ...
  const getNodeTypeIcon = (nodeType: string) => {
    switch (nodeType) {
      case 'class': return <Layers className="h-3 w-3" />;
      case 'function': return <Code2 className="h-3 w-3" />;
      case 'method': return <Target className="h-3 w-3" />;
      case 'module': return <FileText className="h-3 w-3" />;
      default: return <Code2 className="h-3 w-3" />;
    }
  };

  const groupedNodes = context_nodes.reduce((acc, node) => {
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-xs">{contextMetadata.selection_strategy}</span>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <div className="flex justify-between">
                  ...
                  {Object.entries(groupedNodes).map(([file, nodes]) => (
                    <div key={file} className="space-y-2">
                      ...
                      <div className="space-y-1 ml-5">
                        {nodes.map((node) => (
                          <div key={node.node_id} className="flex items-center justify-between p-2 rounded-md bg-muted/50 hover:bg-muted">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <Tooltip>
                                ...
                                <TooltipContent>
                                  ...
                                  <p className="text-xs text-muted-foreground">{node.inclusion_reason}</p>
                                </TooltipContent>
                              </Tooltip>
                            </div>
                          </div>
