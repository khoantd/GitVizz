...
const PropertiesView = ({ nodeId, edgeId, graph }: PropertiesViewProps) => {
  const [properties, setProperties] = useState<NodeProperties | EdgeProperties | null>(null);
  ...
  useEffect(() => {
    ...
    if (nodeId && graph.hasNode(nodeId)) {
      ...
      try {
        ...
        neighbors.forEach((neighborId: string) => {
          ...
          edges.forEach((edgeId: string) => {
            ...
            const direction = graph.source(edgeId) === nodeId ? 'outgoing' : 'incoming';
      ...
      // Extract module from id if it contains a colon (module:name format)
  ...
  return (
    <div className={`${bgColor} ${textColor} rounded-lg border ${borderColor} p-4 text-sm shadow-lg max-w-xs`}>
      {isNode ? (
        // Node properties view
        <div className="flex flex-col gap-2">
          <h3 className={`text-lg font-bold ${headerColor}`}>
            {(properties as NodeProperties).type.charAt(0).toUpperCase() + (properties as NodeProperties).type.slice(1)}
          </h3>
          ...
          <div className="flex flex-col gap-1">
            ...
            {(properties as NodeProperties).module && (
              <div className="flex items-center gap-1">
                <span className="font-semibold">Module:</span>
                <span className="text-xs overflow-hidden text-ellipsis">{(properties as NodeProperties).module}</span>
              </div>
            )}
            
            <div className="flex items-center gap-1">
              <span className="font-semibold">ID:</span>
              <span className="text-xs overflow-hidden text-ellipsis">{(properties as NodeProperties).id}</span>
            </div>
          </div>
          
          {(properties as NodeProperties).connections && (properties as NodeProperties).connections!.length > 0 && (
            <>
              <h4 className={`text-md font-semibold mt-2 ${sectionColor}`}>Connections</h4>
              <div className="max-h-40 overflow-y-auto">
                {(properties as NodeProperties).connections!.map((conn, idx) => (
                  <div key={idx} className="flex flex-col py-1 border-b border-gray-200 dark:border-gray-700 last:border-0">
                    <div className="flex items-center gap-1">
                      <span className="font-semibold">{conn.label}</span>
                      <span className="text-xs">({conn.type})</span>
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      {conn.direction === 'outgoing' ? 'Calls' : 'Called by'}
                    </div>
                  </div>
                ))}
              </div>
