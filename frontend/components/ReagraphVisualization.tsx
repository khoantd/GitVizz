...
import type { GraphResponse, GraphNode as ApiGraphNode, GraphEdge as ApiGraphEdge } from "@/api-client/types.gen"
    ...
    const fetchGraph = async () => {
      ...
      try {
        ...
        if (!isCancelled) {
          ...
          apiCache.set(requestKey, { data: transformedData, timestamp: Date.now() })
      ...
      <div className="flex justify-center items-center h-full">
        <div className="flex flex-col items-center gap-4 p-8">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Setting up enhanced graph visualization...</p>
          </div>
        </div>
      </div>
    )
