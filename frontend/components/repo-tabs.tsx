    ...
    const handleInstallationCheck = async () => {
      ...
      try {
        ...
        if (installationsData.installations?.length) {
          ...
          const transformedRepos = (reposData.repositories || []).map((repo: any) => ({
            ...repo,
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="space-y-6 sm:space-y-8">
        <Tabs defaultValue="github" className="space-y-6 sm:space-y-8" onValueChange={setActiveMainTab}>
          {/* Tab Navigation */}
          ...
          {/* GitHub Tab */}
          <TabsContent value="github" className="animate-in fade-in-50 duration-300 max-h-[70vh] overflow-y-auto">
              ...
              <div className="p-4 sm:p-8">
                <form onSubmit={handleGitHubSubmit} className="space-y-4 sm:space-y-6">
                  <div className="space-y-2 sm:space-y-3">
                    ...
                    <Input id="repo-url" placeholder="https://github.com/username/repository" value={repoUrl} onChange={(e) => setRepoUrl(e.target.value)} className="h-10 sm:h-12 rounded-xl border-border/50 bg-background/50 backdrop-blur-sm text-sm sm:text-base" required />
                  </div>
                  <div className="flex items-center justify-between">
                    <Button type="button" variant="ghost" size="sm" onClick={() => setShowAdvanced(!showAdvanced)} className="rounded-xl text-muted-foreground hover:text-foreground text-xs sm:text-sm">
                    ...
                    <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                      <div className="grid gap-4 sm:grid-cols-2">
                        <div className="space-y-2 sm:space-y-3">
                          ...
                          <Input id="access-token" type="password" placeholder="ghp_xxxxxxxxxxxx" value={accessToken || ""} onChange={(e) => setAccessToken(e.target.value)} className="h-10 sm:h-12 rounded-xl border-border/50 bg-background/50 backdrop-blur-sm text-sm sm:text-base" />
                        </div>
                        <div className="space-y-2 sm:space-y-3">
                          <Label htmlFor="branch" className="text-sm font-medium"> Branch </Label>
                          <Input id="branch" placeholder="main" value={branch || ""} onChange={(e) => setBranch(e.target.value)} className="h-10 sm:h-12 rounded-xl border-border/50 bg-background/50 backdrop-blur-sm text-sm sm:text-base" />
                        </div>
                      </div>
                      <div className="bg-muted/30 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-border/30">
                        <div className="flex items-start gap-3">
                          <div className="p-1.5 rounded-full bg-blue-500/10 flex-shrink-0"><Info className="h-3 w-3 sm:h-4 sm:w-4 text-blue-500" /></div>
                          <div className="space-y-1 flex-1 min-w-0">
                            <p className="text-xs sm:text-sm font-medium">Privacy & Security</p>
                            <p className="text-xs text-muted-foreground leading-relaxed">Your access token is stored locally and only sent to GitHub. It's never shared with our servers. <a href="https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token" target="_blank" rel="noopener noreferrer" className="text-primary underline hover:no-underline">How to create a GitHub token</a></p>
                          </div>
                        </div>
                      </div>
                    </div>
                  )}
                  <Button type="submit" disabled={loading || !repoUrl.trim()} className="w-full h-10 sm:h-12 text-sm sm:text-base rounded-xl bg-primary hover:bg-primary/90 transition-all duration-200 hover:scale-[1.02]" size="lg">
                    {loading && activeMainTab === 'github' ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin" />Processing...</>) : (<><Zap className="h-4 w-4 mr-2" /><span className="hidden xs:inline">Analyze Repository</span><span className="xs:hidden">Analyze</span><ArrowRight className="h-4 w-4 ml-2" /></>)}
                  </Button>
                </form>
              </div>
            </div>
          ...
          {/* Upload Tab */}
          <TabsContent value="upload" className="animate-in fade-in-50 duration-300 max-h-[70vh] overflow-y-auto">
                ...
                <div className="px-4 sm:px-8 py-4 sm:py-6 border-b border-border/30 bg-gradient-to-r from-primary/5 via-transparent to-transparent">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs sm:text-sm text-muted-foreground mt-1">Upload a ZIP file containing your project to analyze its structure</p>
                        </div>
                    </div>
                </div>
                <div className="p-4 sm:p-8">
                    <form onSubmit={handleZipSubmit} className="space-y-4 sm:space-y-6">
                        ...
                        <div className={cn("relative border-2 border-dashed rounded-xl sm:rounded-2xl p-6 sm:p-12 text-center transition-all duration-300", dragActive ? "border-primary bg-primary/5 scale-[1.02]" : "border-border/50 hover:border-primary/50 hover:bg-muted/20")} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
                            <input ref={fileInputRef} type="file" accept=".zip" onChange={handleFileInput} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"/>
                            <div className="space-y-3 sm:space-y-4">
                                ...
                                <div className="space-y-1 sm:space-y-2">
                                    <p className="text-sm sm:text-lg font-medium">{dragActive ? "Drop your ZIP file here" : "Click to browse or drag and drop"}</p>
                            ...
                            <div className="bg-muted/30 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-border/30 animate-in slide-in-from-top-2 duration-300">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs sm:text-sm text-muted-foreground">{(zipFile.size / 1024 / 1024).toFixed(2)} MB</p>
                                    </div>
                                    <Badge className="bg-green-500/10 text-green-700 dark:text-green-300 border-green-500/20 text-xs">Ready</Badge>
                                    <Button type="button" variant="ghost" size="icon" onClick={() => setZipFile(null)} className="h-6 w-6 sm:h-8 sm:w-8 rounded-full hover:bg-muted/50 flex-shrink-0">
                                        <X className="h-3 w-3 sm:h-4 sm:w-4" />
                                    </Button>
                                </div>
                            </div>
                        )}
                        <Button type="submit" disabled={loading || !zipFile} className="w-full h-10 sm:h-12 text-sm sm:text-base rounded-xl bg-primary hover:bg-primary/90 transition-all duration-200 hover:scale-[1.02]" size="lg">
                            {loading && activeMainTab === 'upload' ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin" />Processing...</>) : (<><Zap className="h-4 w-4 mr-2" /><span className="hidden xs:inline">Process ZIP File</span><span className="xs:hidden">Process</span><ArrowRight className="h-4 w-4 ml-2" /></>)}
                        </Button>
                    </form>
                </div>
            </div>
            ...
            <div className="bg-background/60 backdrop-blur-xl border border-border/50 rounded-2xl sm:rounded-3xl shadow-sm overflow-hidden">
                {isReposLoading ? (
                    ...
                    <>
                        <div className="px-4 sm:px-8 py-4 sm:py-6 border-b border-border/30 bg-gradient-to-r from-primary/5 via-transparent to-transparent">
                            <div className="flex items-center gap-3">
                                <div className="p-2 sm:p-2.5 rounded-xl sm:rounded-2xl bg-primary/10"><Github className="h-4 w-4 sm:h-5 sm:w-5 text-primary" /></div>
                                <div>
                                    <h3 className="text-lg sm:text-xl font-semibold tracking-tight">Connect to GitHub</h3>
                                    <p className="text-xs sm:text-sm text-muted-foreground mt-1">Install our GitHub App to select from your repositories</p>
                                </div>
                            </div>
                        </div>
                        <div className="p-4 sm:p-8">
                            <div className="flex flex-col items-center justify-center py-8 text-center">
                                <div className="rounded-full bg-primary/10 p-4 mb-6"><Github className="h-8 w-8 text-primary" /></div>
                                <h2 className="text-2xl font-semibold mb-2">Connect Your Repositories</h2>
                                <p className="text-muted-foreground max-w-md mb-8">Install the GitViz GitHub App to securely access and analyze your repositories with one click.</p>
                                <Button size="lg" className="w-full max-w-sm h-12 text-base rounded-xl bg-primary hover:bg-primary/90 transition-all duration-200 hover:scale-[1.02]" onClick={handleInstallApp}>
                                    <Github className="h-5 w-5 mr-2" /> Set up GitHub App <ArrowRight className="h-4 w-4 ml-2" />
                                </Button>
                            </div>
                        </div>
                    </>
                    ...
                    <div className="p-4 sm:p-8 space-y-6">
                        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                            <div className="relative w-full sm:max-w-md">
                                <Input placeholder="Search repositories..." className="w-full h-10 rounded-xl border-border/50 bg-background/50 backdrop-blur-sm pl-9" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                                <Code className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                            </div>
                            <Button variant="outline" className="h-10 px-4 text-sm rounded-xl hover:bg-muted/50 transition-all duration-200 self-end sm:self-auto" onClick={handleManageAccess}>
                        ...
                        <Tabs defaultValue="all" className="w-full" onValueChange={setActiveRepoFilter}>
                            <div className="flex justify-center border-b border-border/30">
                                <TabsList className="bg-transparent p-0 h-auto">
                                    <TabsTrigger value="all" className="text-sm data-[state=active]:shadow-none data-[state=active]:text-primary data-[state=active]:bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none">
                                        All ({repositories.length})
                                    </TabsTrigger>
                                    <TabsTrigger value="public" className="text-sm data-[state=active]:shadow-none data-[state=active]:text-primary data-[state=active]:bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none">
                                        Public ({repositories.filter(r => !r.private).length})
                                    </TabsTrigger>
                                    <TabsTrigger value="private" className="text-sm data-[state=active]:shadow-none data-[state=active]:text-primary data-[state=active]:bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none">
                                ...
                                {filteredRepositories.length > 0 ? (
                                    <div className="divide-y divide-border/30 border border-border/50 rounded-xl overflow-hidden bg-background/20 max-h-[45vh] overflow-y-auto">
                                        {filteredRepositories.map((repo) => (
                                            <div key={repo.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-5 hover:bg-muted/20 transition-colors">
                                                <div className="flex items-center gap-4 mb-3 sm:mb-0 min-w-0">
                                                    <div className="p-2 rounded-xl bg-primary/10 shrink-0">
                                                        {repo.private ? <Lock className="h-4 w-4 text-primary" /> : <Code className="h-4 w-4 text-primary" />}
                                                    </div>
                                                    <div className="min-w-0 flex-1">
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            <a href={repo.html_url} target="_blank" rel="noopener noreferrer" className="text-base font-semibold tracking-tight truncate hover:underline">{repo.name}</a>
                                                            {repo.private && <Badge variant="outline" className="text-xs rounded-md">Private</Badge>}
                                                            {repo.language && <Badge variant="secondary" className="text-xs rounded-md">{repo.language}</Badge>}
                                                        </div>
                                                        <p className="text-xs text-muted-foreground mt-1 line-clamp-1">{repo.description}</p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-2 self-end sm:self-center">
                                                    <Button className="h-9 px-4 text-sm rounded-xl bg-primary hover:bg-primary/90" onClick={() => handleVizifyRepo(repo)} disabled={processingRepos.includes(repo.id) || loading}>
                                                        {processingRepos.includes(repo.id) ? (
                                                            <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Processing...</>
                                                        ) : (
                                                            <><Zap className="h-4 w-4 mr-2" />Vizify</>
                                                        )}
                                                    </Button>
                                                </div>
                                            </div>
                                    ...
                                    <div className="flex flex-col items-center justify-center py-12 text-center border border-dashed rounded-xl">
                                        <div className="rounded-full bg-muted p-3 mb-4"><AlertCircle className="h-6 w-6 text-muted-foreground" /></div>
                                        <h3 className="text-lg font-medium mb-2">No Repositories Found</h3>
                                        <p className="text-muted-foreground max-w-md mb-6">{searchQuery ? `No repositories match your search for "${searchQuery}".` : `No ${activeRepoFilter !== 'all' ? activeRepoFilter : ''} repositories found. Try adding more via "Manage Access".`}</p>
                                    </div>
                                )}
                            </div>
                        </Tabs>
