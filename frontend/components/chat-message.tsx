...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  return (
    <div className={cn("group flex gap-3 w-full", isUser && "flex-row-reverse")}>
      ...
      {/* Message Content */}
      <div className={cn("flex-1 min-w-0 space-y-2 max-w-full", isUser && "flex flex-col items-end")}>
        {/* Message Bubble */}
        <div
          className={cn(
            ...
            <div className="prose prose-sm dark:prose-invert max-w-none w-full">
              <ReactMarkdown
                components={{
                  ...
                  p: ({ children }) => (
                    <p className="text-sm leading-relaxed mb-3 last:mb-0 break-words w-full">{children}</p>
                  ),
                  ...
                  h1: ({ children }) => (
                    <h1 className="text-base font-semibold mb-2 mt-4 first:mt-0 break-words w-full">{children}</h1>
                  ),
                  h2: ({ children }) => (
                    <h2 className="text-sm font-semibold mb-2 mt-3 first:mt-0 break-words w-full">{children}</h2>
                  ),
                  h3: ({ children }) => (
                    <h3 className="text-sm font-medium mb-2 mt-3 first:mt-0 break-words w-full">{children}</h3>
                  ),
                  blockquote: ({ children }) => (
                  ...
                  table: ({ children }) => (
                    <div className="overflow-x-auto my-3 w-full">
                      <table className="min-w-full text-xs border border-border/30 rounded-lg">{children}</table>
                    </div>
                  ),
                  pre: ({ children }) => (
                    <div className="overflow-x-auto w-full">
