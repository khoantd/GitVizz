...
class FormattedTextRequest(BaseModel):
    """Formatted text request model for graph generation."""
    text: str
    use_ast: bool = False
    use_tree_sitter: bool = False
    use_codetext: bool = False
    use_llm: bool = True
...
async def get_graph_data(
    ...
    """
    Return the graph data as JSON for frontend visualization.
    
    Args:
        use_ast: Whether to use the AST parser (default: True)
        use_tree_sitter: Whether to use the Tree-sitter parser (default: False)
        use_codetext: Whether to use the Codetext parser (default: False)
        use_llm: Whether to use the LLM parser (default: False)
    
    Returns:
        JSON object with nodes and edges for the graph
    """
    from parsers.graph_builder import GraphBuilder
    ...
    graph['metadata']['repo_dir'] = repo_dir
    graph['metadata']['parser_config'] = {
        'ast': use_ast,
        'tree_sitter': use_tree_sitter,
        'codetext': use_codetext,
        'llm': use_llm
...
@app.post("/api/graph-data-from-text")
async def get_graph_data_from_text(request: FormattedTextRequest):
    """
    Generate graph data from formatted text input.
    This is particularly useful for LLM-based parsing of code.
    
    Args:
        request: FormattedTextRequest containing the text and parser options
    ...
    config = {
        ...
        'enabled_parsers': {
            'ast': request.use_ast,
            'tree_sitter': request.use_tree_sitter,
            'codetext': request.use_codetext,
            'llm': request.use_llm
        }
    }
    
    # Build the graph using the formatted text
    graph_builder = GraphBuilder(config)
    graph = graph_builder.build_graph(formatted_text=request.text)
    ...
    graph['metadata']['text_length'] = len(request.text)
...
async def fetch_github_structure(repo_request: RepoRequest):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        tree = []
        # GitHub API returns an object with a 'tree' property containing the array of files/dirs
